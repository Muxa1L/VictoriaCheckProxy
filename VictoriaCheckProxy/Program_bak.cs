                                            ...
                                            //, checkEndOfStream: false, leaveOpen: false))
                                            {
                                                ...
                                                var completion = Task.Run(async () =>
                                                {
                                                    ...
                                                    using (var decomp = new DecompressionStream(pipeClient, bufferSize: 100 * 1024 * 1024))
                                                    {
                                                        ...
                                                        var decompressed = ArrayPool<byte>.Shared.Rent(100  * 1024 * 1024); // new byte[1024 * 1024];
                                                        ...
                                                        try
                                                        {
                                                            ...
                                                            while (!isCompleted)
                                                            {
                                                                ...
                                                                {
                                                                    var test = decompressed.Skip(currPos);
                                                                    try
                                                                    {
                                                ...
                                                try
                                                {
                                                    while ((bytesRead = await vmstorStream.ReadAsync(buffer, cts.Token)) > 0)
                                                    {
                                                        ...
                                                        {
                                                            Console.WriteLine($"write to pipe. conn:{pipeServer.IsConnected} cl_h_clsd {pipeServer.ClientSafePipeHandle.IsClosed} cl_h_inv {pipeServer.ClientSafePipeHandle.IsInvalid}");
